...
-- CarkBet butonu için (Luxury Gold stili ile)
...
WHERE p.is_enabled = true
  AND NOT EXISTS (
    ...
    AND pa.href = 'https://t2m.io/CarkBet'
  );

-- BETVSLOT butonu için (Fire Flame stili ile)
INSERT INTO public.page_actions (page_id, label, href, variant, preset, image_url, sort_order, is_enabled)
SELECT 
    p.id AS page_id,
...
WHERE p.is_enabled = true
  AND NOT EXISTS (
    SELECT 1 FROM public.page_actions pa 
    WHERE pa.page_id = p.id 
    AND pa.href = 'https://t2m.io/kOTWHrM'
  );
