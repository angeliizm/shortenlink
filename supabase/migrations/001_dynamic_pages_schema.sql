...
CREATE INDEX idx_pages_site_slug ON public.pages(site_slug) WHERE is_enabled = true;
CREATE INDEX idx_pages_owner_id ON public.pages(owner_id);
CREATE INDEX idx_page_actions_page_id ON public.page_actions(page_id) WHERE is_enabled = true;
CREATE INDEX idx_page_actions_sort_order ON public.page_actions(page_id, sort_order);
CREATE INDEX idx_custom_domains_hostname ON public.custom_domains(hostname) WHERE status = 'verified';
...
BEGIN
  IF p_id IS NULL THEN
    ...
    VALUES (
      ...
      p_target_url, COALESCE(p_meta, '{}'::jsonb), COALESCE(p_is_enabled, true), auth.uid()
    )
...
BEGIN
  ...
  IF p_id IS NULL THEN
    ...
    INSERT INTO public.page_actions (
      page_id, label, href, variant, sort_order, is_enabled
    )
    VALUES (
...
BEGIN
  ...
  INSERT INTO public.page_actions (page_id, label, href, variant, sort_order, is_enabled)
  VALUES 
    (demo_page_id, 'Documentation', 'https://docs.example.com', 'outline', 1, true),
    (demo_page_id, 'GitHub', 'https://github.com', 'ghost', 2, true),
    (demo_page_id, 'Contact', 'https://example.com/contact', 'primary', 3, true);
  ...
  INSERT INTO public.page_actions (page_id, label, href, variant, sort_order, is_enabled)
  VALUES 
    (casino_page_id, 'Register Now', 'https://www.bet365.com/register', 'primary', 1, true),
    (casino_page_id, 'Promotions', 'https://www.bet365.com/promotions', 'outline', 2, true),
    (casino_page_id, 'Live Casino', 'https://www.bet365.com/live', 'outline', 3, true),
    (casino_page_id, 'Support', 'https://www.bet365.com/help', 'ghost', 4, true);
END $$;